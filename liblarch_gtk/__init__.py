...
def brightness(color_str):
    ...
    Courtesy to
    http://stackoverflow.com/questions/596216/formula-to-determine-brightness-
    of-rgb-color
    """
...
class TreeView(Gtk.TreeView):
    ...
    def __init__(self, tree, description):
        ...
        for col_num, (order_num, col_name) in \
            enumerate(sorted(self.order_of_column), 1):
            desc = description[col_name]
...
#            self.treemodel = Gtk.TreeModelSort(treemodelfilter)
    ...
    def collapse_node(self, llpath):
        ...
        if schedule_next:
            self.basetree.queue_action(
                node_id, self.collapse_node, param=llpath
            )
    ...
    def set_sort_column(self, col_name):
        ...
        if ENABLE_SORTING:
            col_num, col = self.columns[col_name]
            self.treemodel.set_sort_column_id(col_num, Gtk.SortType.ASCENDING)
    ...
    def set_bg_color(self, color_func, color_column):
        ...
        def closure_default_color(func, column):
            ...
            """
            default = column.get_tree_view().get_style().base[
    ...
    def __init_dnd(self):
        ...
        if self.dnd_internal_target == '':
            error = (
                "Cannot initialize DND without a valid name\n"
                "Use set_dnd_name() first"
            )
        ...
        for iter in iters:
            if info == 0:
                try:
                    ...
                    # Gtk API does not allow us to test the string
                    dragged_iter = None
